...
class Endpoint:
    ...
    """
    def __init__(self, root_url, path_component, config_id, token):
        """
        Create an endpoint.
        ...
        Args
            ...
            path_component (str): The section of the path specific to the
                endpoint and configuration. eg. "main/tables"
        ...
        self.base_url = '{}/gooddata-writer/v2/{}/{}'.format(root_url.strip('/'),config_id.strip('/'),
                                                  path_component.strip('/'))
        self.token = token
        self._auth_header = {'X-StorageApi-Token': self.token,
                             'Content-Type': 'application/json',
                             'User-Agent': 'GD wr api client'}
